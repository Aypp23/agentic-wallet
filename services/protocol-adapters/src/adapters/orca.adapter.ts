...
const ORCA_DEXES = process.env.ORCA_JUP_DEXES ?? 'Orca V2';
...
const withDexFilter = (url: URL): URL => {
  url.searchParams.set('dexes', ORCA_DEXES);
  return url;
};
...
export const createOrcaAdapter = (): ProtocolAdapter => ({
  name: 'orca',
  version: '1.1.0',
  programIds: [ORCA_PROGRAM],
  ...
  async getSwapQuote(params: SwapQuoteParams): Promise<SwapQuote> {
    if (isDevnetRpc()) {
      return {
        ...
        route: {
          ...
          reason: 'orca_devnet_route_not_available',
        },
      };
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Orca quote failed: ${res.status} ${await res.text()}`);
      }
      ...
      throw new Error(`Orca quote unavailable: ${message}`);
    }
  },
  ...
  async buildSwap(params: SwapExecuteParams): Promise<BuildResult> {
    if (isDevnetRpc()) {
      return {
        ...
        programIds: [ORCA_PROGRAM, MEMO_PROGRAM],
        metadata: {
          mode: 'devnet_compatibility',
          reason: 'orca_devnet_route_not_available',
        },
    ...
    if (!res.ok) {
      throw new Error(`Orca build failed: ${res.status} ${await res.text()}`);
    }
